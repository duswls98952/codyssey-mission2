...
작업 내용:
- `QuizGame.list_quizzes()` 메서드 추가
- 현재 퀴즈 목록의 문제 제목을 번호와 함께 출력
- 퀴즈가 없는 경우 안내 메시지 출력
- 메뉴 3번을 퀴즈 목록 기능과 연결

검증 내용:
- 메뉴 3번 선택 시 기본 퀴즈 5개 목록 출력 확인

증빙 자료:

### 퀴즈 목록 화면
![퀴즈 목록 화면](screenshots/quiz_list.png)
...
작업 내용:
- `play_quiz()`가 맞힌 개수를 반환하도록 수정
- `QuizGame`에 `has_played` 속성 추가
- 퀴즈 풀이 후 최고 점수와 비교하여 갱신
- `QuizGame.update_best_score()` 메서드 추가
- `QuizGame.show_score()` 메서드 추가
- 메뉴 4번을 점수 확인 기능과 연결

검증 내용:
- 퀴즈를 풀기 전 점수 확인 시 안내 메시지 출력 확인
- 퀴즈 풀이 후 최고 점수 갱신 확인
- 메뉴 4번 선택 시 현재 최고 점수 출력 확인
...
- 손상된 `state.json`을 기본 퀴즈 데이터로 복구하는 흐름 확인
